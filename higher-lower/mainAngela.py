...
def game():
  ...
  #account_a = get_random_account() #10 # I removed this steps, it's unnecessary
  account_b = get_random_account() #11

  while game_should_continue:
    account_a = account_b # 11 => 10
    account_b = get_random_account() # 13

    while account_a == account_b: #11 == 13
      account_b = get_random_account()

    print(f"Compare A: {format_data(account_a)}.") #11
    print(vs)
    print(f"Against B: {format_data(account_b)}.") #13
    
    guess = input("Who has more followers? Type 'A' or 'B': ").lower() #a
    a_follower_count = account_a["follower_count"] #150
    b_follower_count = account_b["follower_count"] #250
